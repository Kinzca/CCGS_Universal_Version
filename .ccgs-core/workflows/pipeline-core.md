...
| 触发关键词 | 定向读取 |
|:---|:---|
| 涉及「窗口」「生成」「续关」 | GDD `03`、`04`、`05` |
| 涉及「难度」「心流」「DDA」 | GDD `06`、research 索引 |
| 涉及「数值」「配置」 | GDD `09`、Luban 相关 |
| 涉及「架构」「重构」 | 相关 ADR 文档 |
| 涉及「方法论」「爽感」 | books 索引 |
| 涉及「提案」「审查」 | proposals/ 中的活跃文件 |
...
| 参考资料 | `CCGS-Data/project-docs/books/` |
| 学术研究 | `CCGS-Data/project-docs/research/` |
...
读取 GDD 相关章节交叉校验：窗口语义（`03`）、方块架构（`05`）、可解性（`04`）、心流通道（`06`）、Luban 规范（`09`）、续关真源唯一（`03`）。

> **🔢 可解性强制**：涉及关卡生成变更必须参照 `04` 可解性条款，未通过不得进入 Phase 2。
> **🧠 DDA 强制**：涉及难度参数变更必须参照 `06` 心流模型，偏离 CSR≈1.04 需额外论证。

### 方法论引用（按需）

查阅 `CCGS-Data/project-docs/books/00-books-index.md`，在提案中引用相关方法论并注明出处。涉及学术理论时同时查阅 `CCGS-Data/project-docs/research/00-research-index.md`。
...
**标准操作**：
1. 执行 `/dev-story` 申领任务
2. 严格在其专业领域内进行编码（遵循纯逻辑视图分离、窗口真源唯一、续关真源唯一、验证器只做回放、结构化失败码、数据驱动、模块解耦等原则）
...
### 阶段纪律（当前阶段 C）
- ❌ 严禁后置整窗 continuation probe 拦截首窗
- ❌ 严禁验证阶段临时重建底层路径
- ❌ 严禁将语义 BFS 作为主线生成依赖
- ✅ 先正确性，后多样性

### 架构红线
- `TowerWindowGenerationResult` 窗口唯一真源
- `ContinuationSpec.NextEntrySpec` 续关入口唯一主真源
- `ValidatePrimaryRoute` 只做回放，禁止寻路
- 生成系统必须使用结构化失败码

### 设计红线
- 可解性 > 多样性 > 美术化
- 难度可量化：打乱步数 M 是结构化难度的充分统计量
- 心流通道：CSR ≈ 1.04

### 学术约束
- 毕业设计项目，论文主题围绕"心流理论驱动的动态难度调整"
- 所有 DDA 相关变更必须参照 `CCGS-Data/design/gdd/06-dda-flow-theory.md`
- 所有认知负荷相关设计必须参照 CLT 理论
- 关键学术研究索引：`CCGS-Data/project-docs/research/00-research-index.md`

### 方法论库引用
- 涉及"游戏机制评估"、"爽感设计"或"系统重构"时，优先查阅 `CCGS-Data/project-docs/books/`
- 在提案中引用相关方法论并注明出处
