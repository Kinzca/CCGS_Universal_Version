...
- [ ] Smoke check has been run with a PASS or PASS WITH WARNINGS verdict — report exists in `CCGS-Data/production/qa/`
- [ ] QA plan exists in `CCGS-Data/production/qa/` (generated by `/qa-plan`) covering this sprint or final production sprint
- [ ] QA sign-off report exists in `CCGS-Data/production/qa/` (generated by `/team-qa`) with verdict APPROVED or APPROVED WITH CONDITIONS
...
- [ ] QA test plan exists (`/qa-plan` output in `CCGS-Data/production/qa/`)
